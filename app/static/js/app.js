...
let processingTurn = false; // Prevents concurrent calls to process_turn
...
function renderState(state) {
  console.log(`[RenderDebug] GameID: ${state.game_id}, GameType: ${state.game_type}, CurrentType: ${currentGameType}`);
  console.log(`[RenderDebug] State: ${state.state}, Turn: ${state.turn}`);

  // Update game type if changed
  if (state.game_type && state.game_type !== currentGameType) {
    console.log(`[RenderDebug] Updating game type from ${currentGameType} to ${state.game_type}`);
  ...
  if (currentGameType === 'rock_paper_scissors' || state.game_type === 'rock_paper_scissors') {
    currentGameType = 'rock_paper_scissors';
  ...
  // Handle Tic Tac Toe
  if (currentGameType === 'tic_tac_toe' || state.game_type === 'tic_tac_toe') {
    currentGameType = 'tic_tac_toe';
    if (state.moves && state.moves.length) {
      const newMoves = state.moves.filter(m => m.ply > lastRenderedPly).sort((a, b) => a.ply - b.ply);
      for (const m of newMoves) {
        const sideName = m.side;
    ...
    // Client-driven AI turn execution for TTT
    const isWhiteTurn = state.turn === 'white';
    const isBlackTurn = state.turn === 'black';
    const whiteIsAI = state.white_model && state.white_model !== 'human';
    const blackIsAI = state.black_model && state.black_model !== 'human';
    const aiTurn = (isWhiteTurn && whiteIsAI) || (isBlackTurn && blackIsAI);

    console.log(`[TurnDebug] Turn: ${state.turn}, WhiteModel: ${state.white_model}, BlackModel: ${state.black_model}`);
    console.log(`[TurnDebug] isWhiteTurn: ${isWhiteTurn}, whiteIsAI: ${whiteIsAI}, isBlackTurn: ${isBlackTurn}, blackIsAI: ${blackIsAI}`);
    console.log(`[TurnDebug] aiTurn: ${aiTurn}, Over: ${state.over}, Processing: ${processingTurn}`);

    if (aiTurn && !state.over && !processingTurn) {
      processingTurn = true;
      console.log("[TurnDebug] Triggering process_turn...");
      setTimeout(async () => {
        try {
          console.log(`[TurnDebug] Calling POST /api/games/${state.game_id}/process_turn`);
          const result = await api(`/${state.game_id}/process_turn`, { method: 'POST' });
          console.log("[TurnDebug] process_turn result:", result);
          console.log("[TurnDebug] Waiting for next poll to refresh state");
        } catch (e) {
          console.error("[TurnDebug] Turn processing failed:", e);
          addLog(`Error: ${e.message}`);
        } finally {
          processingTurn = false;
          console.log("[TurnDebug] processingTurn reset to false");
        }
      }, 100);
    }
  ...
  // Client-driven AI turn execution
  const isWhiteTurn = state.turn === 'white';
  const isBlackTurn = state.turn === 'black';
  // Check if current turn side is AI (has a model assigned)
  const whiteIsAI = state.white_model && state.white_model !== 'human';
  const blackIsAI = state.black_model && state.black_model !== 'human';
  ...
  if (aiTurn && !state.over && !processingTurn) {
    processingTurn = true;
    console.log("[TurnDebug] Triggering process_turn...");
    // Small delay to allow UI to render before processing
    setTimeout(async () => {
      try {
        console.log(`[TurnDebug] Calling POST /api/games/${state.game_id}/process_turn`);
        const result = await api(`/${state.game_id}/process_turn`, { method: 'POST' });
        console.log("[TurnDebug] process_turn result:", result);
        // Don't call renderState here - let the polling interval handle it
        // This prevents recursive calls and ensures smooth operation
        console.log("[TurnDebug] Waiting for next poll to refresh state");
      } catch (e) {
        console.error("[TurnDebug] Turn processing failed:", e);
        addLog(`Error: ${e.message}`);
  ...
  try {
    ...
    if (currentGameId) {
      ...
      if (currentState.over) {
        ...
        addLog('Game started'); startPolling(); setControls('running');
        return;
      }

      // Game exists and is not over, just start polling
      // const body = { white_model: whiteSel.value, black_model: blackSel.value };
      // await api(`/${currentGameId}/start_autoplay`, { method: 'POST', body: JSON.stringify(body) });
      addLog('Resuming game'); startPolling(); setControls('running');
      return;
    }
...
resetBtn.addEventListener('click', async () => {
  if (!currentGameId) return; try {
    setControls('busy'); const state = await api(`/${currentGameId}/reset`, { method: 'POST' });
