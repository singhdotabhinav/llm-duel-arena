...
def test_word_association_basic_flow():
    ...
    state_after = _parse_state(engine)
    history = [h for h in state_after.get("history", []) if h.get("black") or h.get("white")]
    assert history
    assert history[0]["white"] == "Lunar mission"
    assert history[0]["black"] == "Apollo guidance"
...
def test_word_association_completion_draw():
    ...
    for idx in range(max_rounds):
        prompt = engine.current_prompt or "prompt"
        keyword = prompt.split()[0]
        assert engine.push_move(f"{keyword} insight {idx}")
        assert engine.push_move(f"{keyword} context {idx}")
