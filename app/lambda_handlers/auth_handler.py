...
def handle_login(query_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "statusCode": 200,
        "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"},
        "body": json.dumps({"auth_url": auth_url, "state": state}),  # Client should verify this on callback
    }
...
def handle_callback(query_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"},
        "body": json.dumps(
            {"code": code, "state": state, "message": "Callback received. Exchange code for token on client side."}
        ),
    }
...
def get_user_info(headers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "statusCode": 200,
        "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"},
        "body": json.dumps({"logged_in": True, "user": {"id": "user_id_from_token", "email": "user@example.com"}}),
    }
