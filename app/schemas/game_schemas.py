...
class GameCreateRequest(BaseModel):
    ...
    game_type: str = Field(
        ...,
        min_length=1,
        max_length=MAX_GAME_TYPE_LENGTH,
    ...
    white_model: str = Field(
        ...
        description="Model for white player"
    )
    black_model: str = Field(
        ...,
        min_length=1,
        max_length=MAX_MODEL_NAME_LENGTH,
        description="Model for black player"
    )
    ...
    def validate_game_type(cls, v):
        ...
        allowed_types = [
            "chess", "tic_tac_toe", "rock_paper_scissors",
            "racing", "word_association_clash"
        ]
    ...
    def validate_model_name(cls, v):
        ...
        if not any(v.startswith(prefix) for prefix in ALLOWED_MODEL_PREFIXES):
            raise ValueError(
                f"Invalid model name. Must start with one of: {ALLOWED_MODEL_PREFIXES}"
            )
...
class AutoplayRequest(BaseModel):
    ...
    speed: Optional[float] = Field(
        ...
        ge=0.1,
        le=10.0,
        description="Speed multiplier for autoplay (0.1 to 10.0)"
    )
...
class RandomDuelRequest(BaseModel):
    ...
    game_type: Optional[str] = Field(
        default="chess",
        max_length=MAX_GAME_TYPE_LENGTH,
        description="Type of game for random duel"
    )
