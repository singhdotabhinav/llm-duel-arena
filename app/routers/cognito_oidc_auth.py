...
register_kwargs = {
    ...
    'client_kwargs': {
        'scope': settings.cognito_scopes  # These must match Cognito App Client allowed scopes
    },
}
...
async def login(request: Request):
    ...
    if 'localhost' not in redirect_uri:
        ...
        logger.error(f"[Cognito OIDC] Cognito requires localhost for HTTP callbacks. Please update COGNITO_CALLBACK_URL in .env")
        raise HTTPException(
            status_code=500,
            detail=(
...
async def authorize(request: Request):
    ...
    try:
        ...
        if not hasattr(request, 'session'):
            ...
            raise HTTPException(status_code=500, detail="Session not available. Please ensure SessionMiddleware is configured.")
...
async def debug_session(request: Request):
    ...
    session_data["note"] = "Session cookie should be set by SessionMiddleware. Check browser DevTools → Network → Response Headers → Set-Cookie"
